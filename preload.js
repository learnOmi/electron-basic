...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  // Shell 功能
  shellOpenExternal: (url) => ipcRenderer.send('shell-open-external', url),
  shellShowItemInFolder: (path) => ipcRenderer.send('shell-show-item-in-folder', path),
  shellTrashItem: (path) => ipcRenderer.send('shell-trash-item', path),
  onShellResult: (callback) => ipcRenderer.on('shell-result', callback),
